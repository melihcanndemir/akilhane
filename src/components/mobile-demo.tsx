...
export default function MobileDemo() {
  ...
  const pwaFeatures = [
    { name: 'Offline Support', status: 'Active', icon: WifiOff, color: 'green' },
    { name: 'Installable', status: 'Available', icon: Download, color: 'blue' },
    { name: 'App-like Experience', status: 'Enabled', icon: Smartphone, color: 'purple' },
    { name: 'Fast Loading', status: 'Optimized', icon: Zap, color: 'orange' },
  ];

  const touchGestures = [
    { name: 'Swipe Navigation', description: 'Flashcard swipe gestures', icon: ArrowRight },
    { name: 'Tap Interactions', description: 'Touch-friendly buttons', icon: Fingerprint },
    { name: 'Pinch to Zoom', description: 'Image zoom support', icon: Maximize2 },
    { name: 'Touch Feedback', description: 'Haptic feedback', icon: Hand },
  ];

  const mobileOptimizations = [
    { metric: 'Touch Target Size', value: '44px+', status: 'Optimal', color: 'green' },
    { metric: 'Loading Speed', value: '0.8s', status: 'Fast', color: 'blue' },
    { metric: 'Battery Usage', value: 'Optimized', status: 'Efficient', color: 'purple' },
    { metric: 'Data Usage', value: 'Minimal', status: 'Compressed', color: 'orange' },
  ];
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Progressive Web App özellikleri, responsive tasarım ve touch gesture desteği ile mobil-öncelikli deneyim
            </p>
          </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="p-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg">
                          ...
                          <div className="text-xs text-muted-foreground">PWA Score</div>
                        </div>
                      </div>
                    </div>
                  </CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {pwaFeatures.map((feature) => (
              <div key={feature.name} className="border-gradient-purple p-[1px] transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 rounded-xl">
                <Card className="h-full w-full border-0 bg-gradient-to-br from-purple-500/10 to-pink-500/10 dark:from-purple-500/20 dark:to-pink-500/20 rounded-[11px]">
                  <CardHeader className="text-center">
                    <div className={`w-12 h-12 bg-gradient-to-br from-${feature.color}-600 to-${feature.color}-700 rounded-lg flex items-center justify-center mb-4 mx-auto`}>
                  ...
                  <CardContent className="text-center">
                    <Badge variant="secondary" className={`bg-${feature.color}-100 text-${feature.color}-800 dark:bg-${feature.color}-900 dark:text-${feature.color}-200`}>
                      {feature.status}
                    </Badge>
                  </CardContent>
                </Card>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {touchGestures.map((gesture) => (
                      <div key={gesture.name} className="flex items-center justify-between p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                        <div className="flex items-center gap-3">
                          <gesture.icon className="h-5 w-5 text-purple-600 dark:text-purple-400" />
                          <div>
                            <div className="font-medium text-foreground">{gesture.name}</div>
                ...
                <CardContent>
                  <div className="space-y-4">
                    {mobileOptimizations.map((optimization) => (
                      <div key={optimization.metric} className="flex items-center justify-between p-3 bg-pink-50 dark:bg-pink-900/20 rounded-lg">
                        <div className="flex items-center gap-3">
                          <div className={`w-3 h-3 bg-${optimization.color}-500 rounded-full`}></div>
                          <span className="font-medium text-foreground">{optimization.metric}</span>
                        </div>
                        <div className="text-right">
                          <div className="font-semibold text-foreground">{optimization.value}</div>
                          <Badge variant="secondary" className={`bg-${optimization.color}-100 text-${optimization.color}-800 dark:bg-${optimization.color}-900 dark:text-${optimization.color}-200`}>
                            {optimization.status}
                          </Badge>
                        </div>
                      </div>
