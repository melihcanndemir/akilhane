...
const QuizComponent: React.FC<QuizProps> = ({ subject, isDemoMode = false }) => {
  ...
  const [userAnswers, setUserAnswers] = useState<Array<{ questionId: string; topic: string; isCorrect: boolean }>>([]);
    ...
    const loadQuestions = async () => {
      try {
        ...
        if (demoModeActive) {
                ...
                return [
                  {
                    ...
                    explanation: 'Hız = Yol / Zaman → Yol = Hız × Zaman = 20 × 5 = 100 m',
                  },
                  {
                    id: 'demo_physics_2',
                    subject: 'Fizik',
                    ...
                    explanation: 'Dünya\'da yerçekimi ivmesi yaklaşık 9.8 m/s²\'dir.',
                  },
                  {
                    id: 'demo_physics_3',
                    subject: 'Fizik',
                    ...
                    explanation: 'Kinetik Enerji = ½ × m × v² → 100 = ½ × 2 × v² → v² = 100 → v = 10 m/s',
                  },
                ];
              case 'Kimya':
                return [
                  {
                    ...
                    explanation: 'H₂O su molekülünde 2 hidrojen (H) ve 1 oksijen (O) atomu vardır.',
                  },
                  {
                    id: 'demo_chemistry_2',
                    subject: 'Kimya',
          ...
          const questionCount = Math.min(userSettings.studyPreferences.questionsPerQuiz, demoQuestions.length);
        ...
        const getQuestionsFromStorage = (): Question[] => {
          ...
          try {
            ...
            return questions.filter((q: unknown) =>
              typeof q === 'object' && q !== null && 'subject' in q && q.subject === subject,
            ) as Question[];
          } catch {
            return [];
          }
        ...
        // Apply user settings for question count
        const questionCount = Math.min(userSettings.studyPreferences.questionsPerQuiz, localQuestions.length);
  ...
  }, [subject, userSettings.studyPreferences.questionsPerQuiz, userSettings.studyPreferences.timeLimit]);
            ...
            <div className="flex items-center gap-4">
              ...
              {timeLimit && timeRemaining !== null && (
                <span className={`px-3 py-1 rounded-full flex items-center gap-1 ${
                  ...
                  timeRemaining <= 300 ? 'bg-yellow-500 text-white' :
